...
if [ ! -f ${RESAMP_SCP_FILE_TRAIN} ]; then
    ...
    awk -v additional="fsd50k_" '{print additional $1, $2, $3}' $RESAMP_SCP_FILE_TRAIN > ./temp && mv ./temp $RESAMP_SCP_FILE_TRAIN
else
...
if [ ! -f ${RESAMP_SCP_FILE_VALID} ]; then
    ...
    OMP_NUM_THREADS=1 python utils/resample_to_estimated_bandwidth.py \
        ...
        --chunksize 1000
    # add dataset name to make utt-ids unique
    awk -v additional="fsd50k_" '{print additional $1, $2, $3}' $RESAMP_SCP_FILE_VALID > ./temp && mv ./temp $RESAMP_SCP_FILE_VALID
else
