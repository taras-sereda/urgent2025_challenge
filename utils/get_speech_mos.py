    ...
    for uid, dic in tqdm(data.items()):
        ...
        if "start" in dic and "end" in dic:
            ...
            with torch.no_grad():
                ...
                utmos_score = utmos_model(
                    torch.from_numpy(audio)
                    .to(torch.float32)
                    .unsqueeze(0)
                    .to(device=utmos_model.device),
                    fs,
                )
                # NOTE: audio input is not segmented for WVMOS
                # wvmos_score = wvmos_model(dic["audio_path"])
        else:
            with torch.no_grad():
        ...
        mos[uid] = {
            ...
            "UTMOS": float(utmos_score.mean()),
            # f"WVMOS": float(wvmos_score),
        }
    return mos
...
if __name__ == "__main__":
    ...
    # wvmos_model = get_wvmos(cuda=args.use_gpu)
